...
Ray::ResourceSet Task_Test_Request({ { CPU, 1.0 }, { MEM, 1.0 }, { PLASMA_MEM, 8 } });
...
void SumSuccess(std::shared_ptr<Ray::RayEnv> env,int a,int b,int instructNum){
...
void TestTaskSuccess_Driver(NodeID nodeid,int a,int b,int instructNum){
    rayServices[nodeid]->remote(Task_Test_Request,SumSuccess, a, b, instructNum);
