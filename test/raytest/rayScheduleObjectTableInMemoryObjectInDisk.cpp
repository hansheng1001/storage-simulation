...
#include"Tasks_test_ObjectInMemoryObjectLargerInDisk.h"
...
TEST(SetNodeInfo, testGlobalInfo) {
    ...
    NodeMonitor<ArchiveNodeInfo<std::ofstream>, std::ofstream> nm(std::move(AccountNodeInfoFile), 10);
    ...
    EventLoop::getInstance().callAfter(1000, [&nm]()
    {
        nm.stopMonitoring();
    });
    ...
    TestTaskObjectInDisk_Driver(1, 3, 4, 1000);
    ...
    //EventLoop::getInstance().ChooseEventToStart();
    //先将LinkManager删除
    // ManagerL5inkUnOrderMap::getIntance().clear();
